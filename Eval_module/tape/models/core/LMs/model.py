...
from tape.models.core.utils import init_random_state


class BertClassifier(PreTrainedModel):
    def __init__(self, model, n_labels, dropout=0.0, seed=0, cla_bias=True, feat_shrink=''):
        # 避免 transformers 在旧版 torch 上自动启用 SDPA 导致报错，
        # 尽量强制使用 eager 实现
        config = model.config
        try:
            ...
            setattr(config, "_attn_implementation_auto_set", True)
        except Exception:
            pass
...
class BertClaInfModel(nn.Module):
    """
    仅用于推理阶段，将已训练好的 BertClassifier 用于生成节点级 embedding 和 logits，
    不需要继承 PreTrainedModel，避免 transformers 在 __init__ 中自动处理 SDPA。
    """
