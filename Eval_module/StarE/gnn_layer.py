...
from .utils_gcn import get_param, ccorr, rotate, softmax
...
def scatter_add(src, index, dim=0, dim_size=None):
    if src.numel() == 0:
        if dim_size is None:
            raise ValueError("Cannot infer dim_size for empty src without dim_size.")
        shape = list(src.shape)
        shape[dim] = dim_size
        return torch.zeros(shape, device=src.device, dtype=src.dtype)
