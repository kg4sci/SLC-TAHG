...
class StarEEncoder(StarEBase):
    def __init__(
        ...
        if self.use_node_features:
            if node_feat_dim is None:
                raise ValueError("node_feat_dim must be provided when node_features is not None")
            if not isinstance(node_features, torch.Tensor):
                node_features = torch.tensor(node_features, dtype=torch.float32)
            self.node_features = node_features.to(self.device)
            self.node_feat_proj = nn.Linear(node_feat_dim, self.emb_dim)
    ...
    def forward_base(self, sub, rel, drop1, drop2,
        ...
        :return:
        """
        # Prepare input node embeddings
        if self.use_node_features:
            x_input = self.node_feat_proj(self.node_features)
        else:
        ...
        if not self.triple_mode:
            if self.qual_mode == "full":
                ...
                x, r = self.conv1(x=x_input, edge_index=self.edge_index,
